...
def inputData():
    name = input("Сәлем атыңызды енгізіңіз: ")
    number = input("Нөмеріңізді енгізіңіз: ")
    cur.execute(' INSERT INTO postgres.public.phone_book("PersonName", "PhoneNumber") VALUES( %s, %s); ' , (name, number))

def importFromCSV():
    with open("qwer.csv", 'r') as file:
...
def queryData():
    ...
    data = cur.fetchall()
    path = r"C:\Users\lenovo\Desktop\pp\ppsis10\phonebook\personname.txt"
